...
func wordListToRegex(wordList string) (regexStr string) {
	...
	// 构造正则时针对每个关键词调用getPattern
	var patterns []string
	for _, key := range split {
		patterns = append(patterns, getPattern(key))
	}
	regexStr = fmt.Sprintf(`(?mi)%s`, strings.Join(patterns, "|"))
...
func getPattern(key string) string {
	if isChineseChar(key) {
		return fmt.Sprintf(`(?U)(%s)`, key)
	} else {
		return fmt.Sprintf(`\b(%s)\b`, key)
	}
}
